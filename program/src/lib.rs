...
fn process_instruction(
    ...
    msg!("Didnot find the entrypoint required");
...
fn withdraw(
    ...
    let campain_data =CampainDetails::try_from_slice(*writing_account.data.borrow())
        .expect("Error deserializing data");

    if campain_data.admin != *admin_account.key {
        msg!("Only the account admin can withdraw");
        return Err(ProgramError::InvalidAccountData)
    }

    let input_data = WithdrawRequest::try_from_slice(&instruction_data)
        .expect("Instruction Data serialization didnot worked");
...
fn donate(
    ...
    let mut campain_data = CampainDetails::try_from_slice(*writing_account.data.borrow())
        .expect("Error deserializing data");

    campain_data.amount_donated += **donator_program_account.lamports.borrow();
    ...
    **donator_program_account.try_borrow_mut_lamports()? =0;
    campain_data.serialize(&mut &mut writing_account.data.borrow_mut()[..])?;
